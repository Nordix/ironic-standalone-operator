...
func newPrometheusExporterContainer(versionInfo VersionInfo, ironic *metal3api.Ironic, volumeMount corev1.VolumeMount) corev1.Container {
	...
	// Only add HTTP readiness probe when the bind address is reachable from outside the pod.
	// When bound to localhost (127.0.0.1 or ::1), the kubelet cannot reach it for health checks.
